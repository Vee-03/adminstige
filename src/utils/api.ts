...
export async function apiCall<T>(
  ...
  if (token) {
    ...
    console.warn('[apiCall] No admin token found in localStorage (key: admin_token). Requests will be unauthenticated.')
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message = isJson ? parsed?.message || response.statusText : text || response.statusText
      // If backend indicates unauthenticated, remove stored token to prompt re-login
      const unauthenticatedDetected = response.status === 401 || (isJson && typeof parsed?.message === 'string' && parsed.message.toLowerCase().includes('unauthenticated'))
      if (unauthenticatedDetected) {
        console.warn('[apiCall] Server returned unauthenticated. Removing stored admin_token.')
