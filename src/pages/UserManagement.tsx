...
export default function UserManagement() {
  ...
  const showUserDetailAlert = (user: User) => {
    ...
    let htmlContent = `
      <div style="text-align: left;">
        <div style="margin-bottom: 20px;">
          <h3 style="margin: 0 0 5px 0; color: #111; font-size: 16px; font-weight: 600;">Status</h3>
          <span style="display: inline-block; padding: 5px 12px; border-radius: 9999px; font-size: 13px; font-weight: 600; ${
            user.status === "active"
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
          <div>
            <label style="display: block; font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 5px; text-transform: uppercase;">Nama</label>
            <p style="margin: 0; color: #111; font-weight: 500;">${
              user.name || "—"
            }</p>
          </div>
          <div>
            <label style="display: block; font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 5px; text-transform: uppercase;">Dibuat</label>
            <p style="margin: 0; color: #111;">${createdAt}</p>
          </div>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
          <div>
            <label style="display: block; font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 5px; text-transform: uppercase;">Diperbarui</label>
            <p style="margin: 0; color: #111;">${updatedAt}</p>
          </div>
        </div>
    `;
    ...
    // Stats section
    if (
      (user as any).bookings_count !== undefined ||
      (user as any).checkouts_count !== undefined
    ) {
      htmlContent += `
        <div style="background: linear-gradient(to right, #f0f9ff, #f0f4ff); padding: 15px; border-radius: 8px; border: 1px solid #dbeafe; margin-bottom: 20px;">
          <h3 style="margin: 0 0 10px 0; font-weight: 600; color: #111; font-size: 14px;">Statistik</h3>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
      `;
      if ((user as any).bookings_count !== undefined) {
        htmlContent += `
            <div style="background: white; padding: 10px; border-radius: 8px; border: 1px solid #bfdbfe;">
              <p style="margin: 0; font-size: 11px; color: #4b5563; font-weight: 600; text-transform: uppercase;">Total Booking</p>
              <p style="margin: 5px 0 0 0; font-size: 20px; font-weight: bold; color: #2563eb;">${
                (user as any).bookings_count
              }</p>
            </div>
        `;
      }
      if ((user as any).checkouts_count !== undefined) {
        htmlContent += `
            <div style="background: white; padding: 10px; border-radius: 8px; border: 1px solid #c7d2fe;">
              <p style="margin: 0; font-size: 11px; color: #4b5563; font-weight: 600; text-transform: uppercase;">Total Checkout</p>
              <p style="margin: 5px 0 0 0; font-size: 20px; font-weight: bold; color: #4f46e5;">${
                (user as any).checkouts_count
              }</p>
            </div>
        `;
      }
    ...
    if (Array.isArray((user as any).roles) && (user as any).roles.length > 0) {
      htmlContent += `
        <div style="margin-bottom: 20px;">
          <label style="display: block; font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 8px; text-transform: uppercase;">Roles</label>
          <div style="display: flex; flex-wrap: wrap; gap: 8px;">
      `;
      (user as any).roles.forEach((role: any) => {
        htmlContent += `<span style="display: inline-block; padding: 5px 12px; background-color: #f3e8ff; color: #6b21a8; border-radius: 9999px; font-size: 13px; font-weight: 500;">${
          role.name || role
        }</span>`;
      });
      htmlContent += `</div></div>`;
    ...
    if (user.email_verified_at !== undefined) {
      ...
      htmlContent += `
        <div style="display: flex; align-items: center; gap: 10px; padding: 10px; background-color: #f9fafb; border-radius: 8px; border: 1px solid #e5e7eb;">
          <div style="width: 8px; height: 8px; border-radius: 50%; background-color: ${
            verified ? "#22c55e" : "#d1d5db"
          };"></div>
          <span style="font-size: 13px; color: #374151;">Email ${
